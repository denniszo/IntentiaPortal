...
function onLoad(){
	...
	for (var i = 0; i < RejReas_Collection.count; i++){
		...
		var wrh = oStd.GetValue("WRH");
		var note = oStd.GetValue("PFOPTN");
		
		if (mvxd.indexOf("1") >= 0){
			if (wrh == "010" && note.substr(0,1) == "T"){
				var opt, opt2;
	...
	for (var i = 0; i < RejReas_Collection.count; i++){
		...
		if (mvxd.indexOf("1") >= 0){
			if (wrh == "010" && emp.substr(0,1) == "E")
				empStr = empStr + emp + ",";
		}else{
			if (wrh == "200" && emp.substr(0,2) == "UE")
				empStr = empStr + emp + ",";
		}
	}
...
String currentTimeStr1 = new Time(System.currentTimeMillis()).toString();
