...
var gWhereString_Bank = "";
var gWhereString_GL = "";
var gEndProsses=0;

function ChangeComDiv(){
    gCONO = document.getElementById("inpCONO").value;
    gDIVI = document.getElementById("inpDIVI").value;
    ...
    table.clear().draw();
    gSelectedRows_BK = [];
    table = $('#tblGLTrans').DataTable();
    table.clear().draw();
    gSelectedRows_GL = [];


    $('#mdChange_CmpDivi').modal('hide');
    /*
    if($("#selBKIDdesc option").length < 1){
        top.alert("אין בנקים פעילים ")
    }else{
        $('#mdChange_CmpDivi').modal('hide');
    }
    */
...
function btnShowRecon() {
    ...
    if (TransType == 1) {//TransType 1 is open
        WhereString1 = " AND BDVONO=0 ";
        //WhereString2 = " AND FG.EGTOCD!='9' ";
        WhereString2 = " AND FG.EGTOCD='' ";
    }
    else {//close
        WhereString1 = " AND BDVONO!=0 ";
        WhereString2 = " AND FG.EGTOCD<>'' ";
    ...
    gWhereString_Bank = WhereString1;
    gWhereString_GL = WhereString2;

    waitingDialog.show(msgloading); // msgloading is a massage
   // window.setTimeout("waitingDialog.show('"+msgloading+"')",1000);
    Go_Transactions();
    //waitingDialog.show('Custom message', {dialogSize: 'sm', progressType: 'warning'});
    //window.setTimeout("Go_Transactions()",1000);
}

function Go_Transactions() {
    gEndProsses=0;
    getBankTRan();
    getGLTran();
    //waitingDialog.hide();
...
function endProsses(){
    ...
    if(gEndProsses>1){
        waitingDialog.hide();
       $("#lblSumALL").text(gBankSum - gGLSum);
    }
}

function getBankTRan(WhereString) {
    if (WhereString == undefined || WhereString == null) {
        WhereString = gWhereString_Bank;
    }
    gBankSum = 0;
    ...
    OrderStr=OrderStr.substr(0, OrderStr.indexOf(";"))
    console.log("OrderStr= "+OrderStr)
    // pageContext.getComponentById("ddlOrder").getValue(); BDCURD,ABS(BDBLAM);TRDATE,ABS(FG.EGCUAM)
    var BankOrderByStr = OrderStr;//" BDCURD,ABS(BDBLAM) ";//OrderStr.substr(0, OrderStr.indexOf(";"));
    ...
    var url = "Queries/sqlGetBankTrans.jsp?params=" + inputFields + "&returncols=" + returncols;
    ...
    //var selectedRows = [];
    gSelectedRows_BK = [];
    // console.log("gSelectedRows_BK= "+gSelectedRows_BK.length)
    $(document).ready(function () {
        ...
        var table = $('#tblBankTrans').DataTable({
            ...
            /*
             ...
             if ($.inArray(data.DT_RowId, gSelectedRows_BK) !== -1 ) {
             console.log("inArray")
            ...
            "columnDefs": [
                {// render
                    ...
                    "render": function (data, type, row, meta) {
                        ...
                        if (data < 0) {
                            return '<span style="color:red;direction: ltr; ">' + data + '</span>';
                        } else {
                            return data;
                        }
                        ...
                        if (type == "display") {// runs more than one - don't
                            ...
                            if (data < 0) {
                                return '<span style="color:red;direction: rtl; ">' + data + '</span>';
                            } else {
                                return data;
                            }
    ...
    /*
     ...
     // alert(aPos);
     // var table = $('#tblBankTrans').DataTable();
     //alert( 'There are'+table.data().length+' row(s) of data in thi table' );
...
function selBankTran() {
    console.log("selBankTran");
...
function getGLTran(WhereString) {
    ...
    var OrderStr2 = $("#ddlOrder").val();
    OrderStr2=OrderStr2.substr(OrderStr2.indexOf(";")+1);
    console.log("OrderStr2= "+OrderStr2);
    var GLOrderByStr = OrderStr2;// " TRDATE,ABS(FG.EGCUAM) ";//OrderStr.substr(OrderStr.indexOf(";") + 1, OrderStr.length);
    ...
    var url = "Queries/sqlGetGLTrans.jsp?params=" + inputFields;
    // prepare the columns for dataTable
    var arr = returncols.split(',');
    var columns = [];
    for (var i in arr) {
 ...
 */
function btnDoRecon(){
   //window.setTimeout("waitingDialog.show('"+txt_RunRecon+"')",500);
    btnDoRecon_Go();
}
function btnDoRecon_Go() {
